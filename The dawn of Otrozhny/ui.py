...
class HudScreen():
    ...
    def draw(self):
        ...
        if self.interactable:
            pygame.draw.rect(self.viewPort, colors.BLACK, [int(self.cursorX * (self.viewPort.get_width()/5) + renderer.HUD_CELL_OFFSET + 5 ), self.viewPort.get_height() , 100, -10])
...
class VerticalList():
    def __init__(self, str_list, px, py):
        ...
        for line in str_list:
            self.objects.append( HudButton(font.size(line)[0], renderer.HUD_CELL_TITLE_FONT_SIZE, line ) )

    def draw(self):
        for buttons in self.objects:
            buttons.redraw()
        for x in range( len(self.objects) ):
            renderer.SCREEN.blit(self.objects[x] , (self.px, self.py + x * renderer.HUD_CELL_TITLE_FONT_SIZE + x * 10) )
...
class MapSelectionScreen(pygame.Surface):
    def __init__(self):
        ...
        self.hud_buttons = [[None for _ in range(3)] for _ in range(5)] #Why not new List[5][3]...
    ...
    def draw(self):
        ...
        for x in range(5):
            for y in range(3):
                self.blit(  
                self.hud_buttons[x][y], (50 +  x*100 + x* renderer.HUD_CELL_OFFSET,100 + y*100 + y*renderer.HUD_CELL_OFFSET)
                )
        
        pygame.draw.circle(self, colors.WHITE, (int(self._pointer_x), int(self._pointer_y) ) , 10)
        renderer.SCREEN.blit(self, (0,0))

    def update(self, dt, events):
        pos = (100 +  self.selected_button_x*100 + self.selected_button_x* renderer.HUD_CELL_OFFSET,150 + self.selected_button_y*100 + self.selected_button_y*renderer.HUD_CELL_OFFSET)
    ...
    def change_selected_button(self, amount, change_x=True):
        self.hud_buttons[self.selected_button_x][self.selected_button_y].set_active(False)
        if change_x:
            ...
            if self.selected_button_y < 0:
                ...
                self.selected_button_y = 0
        self.hud_buttons[self.selected_button_x][self.selected_button_y].set_active(True)
...
class HudButton(pygame.Surface):
    ...
    def set_text(self, text):
        self.text = str(text)
        wrapped_text = textHelpers.wrapline(self.text, self.get_width(), renderer.HUD_CELL_TITLE_FONT_SIZE)
        row = 1
        ...
        for line in wrapped_text:
            textDraw.message_display(self, line, self.get_width()//2 , py, renderer.HUD_CELL_TITLE_FONT_SIZE, self.indexColor[2])
